...
export interface SpawnAreaEvent extends BaseEvent { type: 'spawn_area'; source?: string; position?: { x: number; y: number }; shape: 'circle' | 'cone' | 'half_room'; radius: number; angle?: number; direction?: 'front' | 'back'; element?: DamageType; mechanic?: string; side?: 'north' | 'south'; telegraphDuration: number; duration: number; resolution: { condition: { type: 'player_count'; min?: number; max?: number }; effects: EffectDefinition[] }[]; }
