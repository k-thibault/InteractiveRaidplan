...
export interface SpawnAreaEvent extends BaseEvent { type: 'spawn_area'; source?: string; position?: PositionValue; shape: 'circle' | 'cone' | 'half_room'; radius: number; angle?: number; direction?: 'front' | 'back' | 'nearest_player'; element?: DamageType; mechanic?: string; side?: 'north' | 'south'; label?: string; excludeSource?: boolean; telegraphStyle?: string; telegraphDuration: number; duration: number; resolution: { condition: { type: 'player_count'; min?: number; max?: number }; effects: EffectDefinition[] }[]; telegraphColor?: string; executionColor?: string; }
